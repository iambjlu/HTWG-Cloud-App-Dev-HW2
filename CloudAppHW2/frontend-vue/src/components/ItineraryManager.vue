...
const fetchItineraries = async () => {
    ...
    try {
        ...
        const response = await axios.get(`http://localhost:3000/api/itineraries/by-email/${props.travellerEmail}`);
...
const saveEdit = async () => {
    ...
    try {
        await axios.put(`http://localhost:3000/api/itineraries/${data.id}`, data);
...
const deleteItinerary = async () => {
    ...
    if (window.confirm(`Delete trip "${selectedItinerary.value.title}" ?`)) { 
         try {
            await axios.delete(`http://localhost:3000/api/itineraries/${selectedItinerary.value.id}`);
...
<template>
  ...
  <div class="grid grid-cols-1 gap-6 h-full">
    ...
    <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-200 overflow-y-auto max-h-[80vh]">
      ...
      <ul class="space-y-2" v-if="itineraries.length > 0">
        <li v-for="it in itineraries" :key="it.id" @click="viewDetails(it.id)" 
