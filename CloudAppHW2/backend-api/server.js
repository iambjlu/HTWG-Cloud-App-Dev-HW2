...
// Middleware Setup
app.use(cors({
    origin: 'http://localhost:5173' // Allow Vue frontend access (Vite default port)
}));
...
app.post('/api/itineraries', async (req, res) => {
    const { traveller_email, title, destination, start_date, short_description, detail_description } = req.body;

    if (!title || !destination || !start_date || short_description.length > 80 || !traveller_email) {
        return res.status(400).send({ message: 'Missing required fields, invalid input, or short description too long. (Requires traveller_email)' });
    }
    ...
    try {
        ...
        const [result] = await pool.execute(
            'INSERT INTO itineraries (traveller_id, title, destination, start_date, short_description, detail_description) VALUES (?, ?, ?, ?, ?, ?)',
            [traveller_id, title, destination, start_date, short_description, detail_description]
        );
...
app.put('/api/itineraries/:id', async (req, res) => {
    const { id } = req.params;
    const { title, destination, start_date, short_description, detail_description } = req.body;

    if (!title || !destination || !start_date || short_description.length > 80) {
    ...
    try {
        const [result] = await pool.execute(
            `UPDATE itineraries SET 
                ...
                detail_description = ? 
             WHERE id = ?`,
            [title, destination, start_date, short_description, detail_description, id]
        );
